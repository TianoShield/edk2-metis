/** @file
  ...
  HBFAplus: Local copy from edk2/MdePkg/Library/BasePrintLib/.
  Only change: #include "PrintLibVAOverride.h" added below to fix
  EFIAPI variadic function crashes on Linux/GCC X64.

  AUDIT (2025):
  =============
  Verified byte-identical to edk2 upstream (minus VA override include).
  6 internal worker functions:
    - BasePrintLibFillBuffer:            Buffer fill with stride, EndBuffer check.
    - BasePrintLibValueToString:         INT64 → reversed digit string.
    - BasePrintLibConvertValueToString:  Legacy (no size check, ASSERT only).
    - BasePrintLibConvertValueToStringS: Safe variant with RSIZE_MAX validation
                                         and SAFE_PRINT_CONSTRAINT_CHECK.
    - BasePrintLibSPrintMarker:          Core format engine (~770 lines).
                                         Handles %d/%u/%x/%X/%p/%s/%S/%a/%c/%g/%t/%r/%%.
                                         \n→\r\n translation. Width, precision,
                                         PREFIX_SIGN, PREFIX_BLANK, COMMA_TYPE,
                                         LONG_TYPE, LEFT_JUSTIFY, PREFIX_ZERO.
                                         Supports both VA_LIST and BASE_LIST.
    - BasePrintLibSPrint:                EFIAPI variadic wrapper.
  SAFE_PRINT_CONSTRAINT_CHECK does ASSERT + return — appropriate for fuzzing.
  BasePrintLibConvertValueToString uses bare ASSERT (no return) but is only
  called from non-"S" APIs which are not exposed in current builds.
  NULL arg handling: %s→"<null string>", %g→"<null guid>", %t→"<null time>".
  No modifications needed — production Intel code, correct for rehosting.
**/

#include "PrintLibVAOverride.h"
