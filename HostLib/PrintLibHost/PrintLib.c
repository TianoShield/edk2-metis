/** @file
  ...
  16 public EFIAPI functions — all thin wrappers calling
  BasePrintLibSPrintMarker or BasePrintLibConvertValueToStringS.
  No ASSERT(FALSE).  All public entry points use safe "S" variants
  with SAFE_PRINT_CONSTRAINT_CHECK (ASSERT + graceful return).
  ASSERT_UNICODE_BUFFER macro validates 16-bit alignment on Unicode
  buffers — appropriate sanity check, not a crash risk under fuzzing
  since the framework always provides properly-aligned buffers.
