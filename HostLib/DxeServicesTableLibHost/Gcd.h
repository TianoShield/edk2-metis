/** @file Gcd.h
  GCD Operations and data structures for DxeServicesTableLibHost.

  AUDIT NOTES (HBFAplus host-fuzzing review):
  ============================================
  This header defines the GCD map entry structure, operation codes, attribute
  conversion tables, and declares all Core* functions that implement the DXE
  Services Table GCD interface.

  External symbols declared here (provided by UefiBootServicesTableLibHost):
  - CoreAcquireLock / CoreReleaseLock  - TPL-based lock acquire/release
  - CoreRaiseTpl / CoreRestoreTpl      - Task priority level management
  - CoreFreePool                       - Pool deallocation (calls free())
  - gDxeCoreImageHandle                - DXE core image handle
  - gCpu                               - CPU arch protocol (always NULL on host)
  - mOnGuarding                        - HeapGuard flag (always FALSE on host)
