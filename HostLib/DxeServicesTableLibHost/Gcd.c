/** @file Gcd.c
  GCD (Global Coherency Domain) services for host-based DXE Services Table.

  AUDIT NOTES (HBFAplus host-fuzzing review):
  ============================================

  This file is a direct copy of the real DxeCore GCD implementation from
  MdeModulePkg/Core/Dxe/Gcd/Gcd.c with minimal modifications for host
  compatibility.  It provides fully functional GCD memory and I/O space
  management using linked-list maps — the same data structures and algorithms
  used in a real UEFI firmware.

  This is intentionally NOT a stub implementation.  Many edk2 DXE drivers
  use gDS->AddMemorySpace, gDS->GetMemorySpaceDescriptor, etc. as part of
  their initialization.  Having a real GCD map means these drivers work
  correctly during host-based fuzzing, improving coverage and realism.

  GCD Map Architecture:
  - mGcdMemorySpaceMap: Linked list of EFI_GCD_MAP_ENTRY covering 0..2^48-1
  - mGcdIoSpaceMap:     Linked list of EFI_GCD_MAP_ENTRY covering 0..2^36-1
  - Initially each map contains a single "NonExistent" entry spanning the
    full address space.  Add/Allocate/Remove operations split and merge entries.

  Key internal functions (from real DxeCore):
  - CoreConvertSpace()       — Central dispatcher for add/free/remove/set-attr ops
  - CoreAllocateSpace()      — Allocation engine with multiple search strategies
  - CoreSearchGcdMapEntry()  — Find entries covering a given range
  - CoreInsertGcdMapEntry()  — Split an entry to insert a new range
  - CoreMergeGcdMapEntry()   — Merge adjacent entries with identical attributes
  - CoreCleanupGcdMapEntry() — Post-operation cleanup and merge pass
  - CoreAllocateGcdMapEntry()— Allocate top/bottom pad entries for splits

  Host-specific notes:
  - Locks (mGcdMemorySpaceLock, mGcdIoSpaceLock) use CoreAcquireLock/
    CoreReleaseLock from UefiBootServicesTableLibHost — these are TPL-based
    no-ops in the single-threaded host environment, but maintain API correctness.
  - gCpu is NULL — CoreSetMemorySpaceAttributes returns EFI_NOT_AVAILABLE_YET
    for CPU arch attributes (cache type changes).  This is acceptable since
    host-based fuzzing has no real CPU cache or page table management.
  - AllocatePool/FreePool come from UefiBootServicesTableLibHost (malloc/free).
  - mOnGuarding is always FALSE — HeapGuard doesn't exist in host environment.
  ...
  DEBUG_CODE (
    ...
    for (Index = 0; Index < NumberOfDescriptors; Index++) {
      DEBUG ((DEBUG_GCD, "%a  %016lx-%016lx %016lx %016lx%c\n",
        mGcdMemoryTypeNames[MIN (MemorySpaceMap[Index].GcdMemoryType, EfiGcdMemoryTypeMaximum-1)],
  ...
  )
{
  ...
  DEBUG ((DEBUG_GCD, "  GcdMemoryType   = %a\n", mGcdMemoryTypeNames[MIN (GcdMemoryType, EfiGcdMemoryTypeMaximum-1)]));
