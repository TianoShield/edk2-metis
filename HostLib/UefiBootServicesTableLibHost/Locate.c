/** @file
  ...
  This file provides CoreLocateHandle, CoreLocateHandleBuffer,
  CoreLocateProtocol, and CoreLocateDevicePath for the host-based
  fuzzing environment.  Derived from edk2 MdeModulePkg/Core/Dxe/Hand/Locate.c.

  Differences from upstream:

  1. CoreLocateHandleBuffer calls CoreLocateHandle twice (size-query
     then fill), releasing the lock between calls.  Upstream uses an
     InternalCoreLocateHandle under a single lock hold.  This is a
     known TOCTOU relaxation acceptable in single-threaded host.

  2. FIX: CoreLocateHandleBuffer now frees the buffer on error,
     preventing a memory leak that could accumulate across fuzz
     iterations (matches upstream edk2 behavior).

  3. All three LocateHandle search types (AllHandles, ByProtocol,
     ByRegisterNotify) are fully implemented.

  4. CoreLocateDevicePath uses Device Path Library helpers
     (GetDevicePathSize, IsDevicePathEnd, CompareMem) to find the
     best-matching handle.  Fully functional for any edk2 component.

  Suitable for rehosting any edk2 component that uses
  gBS->LocateHandle / LocateHandleBuffer / LocateProtocol /
  LocateDevicePath.
...
**/
// AUDIT: Matches edk2 DxeCore Locate*.c logic.
// Linear scan instead of OrderedCollection — SAFE for host.
EFI_STATUS
EFIAPI
CoreLocateHandle (
  ...
  )
{
  ...
  *NumberHandles = BufferSize / sizeof(EFI_HANDLE);
  if (EFI_ERROR(Status)) {
    //
    // FIX: Free the allocated buffer on error to prevent memory leak.
    // Upstream edk2 does this; the original code leaked the buffer.
    //
    CoreFreePool (*Buffer);
    *Buffer = NULL;
    *NumberHandles = 0;
