/** @file
  ...
  Copied from edk2/MdeModulePkg/Core/Dxe/Event/Event.c for host-based
  fuzzing.  This is the real DXE Core event logic, gaining for free:
    - SignalCount dedup (CoreSignalEvent only queues if SignalCount==0)
    - Per-TPL linked list queues (gEventQueue[])
    - Event group propagation (CoreNotifySignalList + gEventSignalQueue)
    - Proper IEVENT lifecycle with CoreFreePool

  Changes from upstream:
    - #include "EventDxeCore.h" instead of "Event.h"
    - Runtime event code guarded by HOST_RUNTIME_EVENTS_ENABLED
      (disabled by default — runtime services are out of scope)
    - Close-listener hooks added in CoreCloseEvent
    - Added EfiEventEmptyFunction stub (upstream gets it from UefiLib)
  ...
  )
{
  ...
  // Host environment: always use AllocateZeroPool (no runtime distinction).
  //
  ...
  )
{
  ...
  //
  // HOST FUZZING EXTENSION: Auto-advance simulated time for timer events.
  //
  // On real hardware, the timer ISR fires periodically (every ~10ms) and
  // advances mEfiSystemTime via CoreTimerTick.  Drivers that poll timer
  // events in tight loops (e.g., NvmExpressPassThru §5.7 completion wait)
  // rely on this to eventually signal the timer and break the loop.
  //
  // In the host environment, no ISR fires — simulated time is advanced
  // only by explicit MockTimerAdvance calls.  Without this extension,
  // any driver polling loop of the form:
  //   while (EFI_ERROR(gBS->CheckEvent(TimerEvent))) { ... }
  // would spin forever because the timer never fires.
  //
  // Fix: When CheckEvent is about to return NOT_READY for a timer event,
  // advance simulated time by 1ms (10000 × 100ns).  The advance happens
  // AFTER the normal check path, so the first call sees ground truth and
  // the advancement benefits the NEXT poll iteration.  This matches the
  // ~1ms granularity of real UEFI timer ticks.
  //
  if ((Status == EFI_NOT_READY) && ((Event->Type & EVT_TIMER) != 0)) {
    CoreTimerTick (10000);
  }
  ...
  )
{
  ...
  //
  // Host environment: do a bounded check instead of infinite loop.
  // Real edk2 spins forever; we do a limited pass to avoid hanging.
  //
  {
    UINTN  Iteration;
    UINTN  MaxIterations;
    ...
    //
    // Host: after bounded iterations, return first event to avoid blocking
    //
    if (UserIndex != NULL) {
      *UserIndex = 0;
    }
  ...
  )
{
  ...
  if ((Event->Type & EVT_TIMER) != 0) {
    CoreSetTimer (UserEvent, TimerCancel, 0);
  }
