/** @file
  ...
  This file provides CoreConnectController, CoreDisconnectController,
  CoreConnectSingleController, and AddSortedDriverBindingProtocol for
  the host-based fuzzing environment.  Derived from edk2
  MdeModulePkg/Core/Dxe/Hand/DriverSupport.c.

  Key notes for fuzzing:

  1. ConnectController / DisconnectController are fully implemented
     and follow the UEFI Driver Model protocol.  However, in most
     fuzzing scenarios no EFI_DRIVER_BINDING_PROTOCOL instances are
     installed, so ConnectController returns EFI_NOT_FOUND.

  2. The Security2 authentication check in CoreConnectController is
     active.  If gSecurity2 is NULL (default), the check is skipped.

  3. CoreConnectSingleController implements the full driver-binding
     priority sort including Platform Override, Driver Family Override,
     Bus-Specific Override, and Version-based ordering.

  4. DisconnectController properly handles BY_DRIVER opens and calls
     Stop() on the driver binding protocol.

  These functions are rarely exercised during fuzzing since most fuzz
  harnesses operate at the protocol level (Install/Locate), not the
  driver model level.  They are included for completeness and to
  support any edk2 code that calls ConnectController/DisconnectController.
...
{
  // AUDIT: Full port of edk2 DxeCore ConnectController with BY_DRIVER open,
  // recursive connect, driver binding sort. All protocol callbacks are real —
  // harness relies on this for DrvEntryPoint connectivity.
  EFI_STATUS                           Status;
