/** @file
  ...
  AUDIT: HBFAplus host-side DevicePathLib public API wrappers.
  Source: Derived from edk2 MdePkg/Library/UefiDevicePathLib/UefiDevicePathLib.c.
  This file provides the standard DevicePathLib API (GetDevicePathSize,
  DuplicateDevicePath, AppendDevicePath, etc.) by delegating to the
  UefiDevicePathLib* internal implementations in DevicePathUtilities.c.

  Text-conversion stubs:
    ConvertDeviceNodeToText, ConvertDevicePathToText, ConvertTextToDeviceNode,
    ConvertTextToDevicePath all return NULL.  The upstream implementation is
    ~4,500 lines of pure string parsing/generation that adds no fuzzing value.
    Callers that rely on text conversion will see NULL and should handle it
    gracefully; any code-under-test that crashes on NULL return reveals a
    real robustness bug.

  Suitability: Fully suitable for rehosting & fuzzing.
**/
