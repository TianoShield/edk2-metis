/** @file
  SMM protocol notification infrastructure — Host-based implementation.

  Copyright (c) 2009 - 2018, Intel Corporation. All rights reserved.<BR>
  SPDX-License-Identifier: BSD-2-Clause-Patent

  ## AUDIT ##
  Faithful copy of edk2/MdeModulePkg/Core/PiSmmCore/Notify.c.
  Implements notification services for SMM protocol installs/uninstalls.

  Public EFIAPI (wired into SMST):
    SmmRegisterProtocolNotify — register/unregister notification callbacks.
      - Function != NULL → register a new notification
      - Function == NULL → unregister an existing notification (via Registration)
      - Duplicate detection: if same (protocol, function) already registered,
        returns existing registration without creating a duplicate.

  Internal helpers (called by Handle.c):
    SmmNotifyProtocol             — invoke all registered callbacks for a protocol
    SmmRemoveInterfaceFromProtocol — fix up notify positions when interface removed

  No modifications needed — logic matches upstream and is correct for rehosting.
**/
