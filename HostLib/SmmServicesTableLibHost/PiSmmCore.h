/** @file
  ...
  ## AUDIT ##
  Host-side version of edk2/MdeModulePkg/Core/PiSmmCore/PiSmmCore.h.
  Defines all internal data structures for the SMM protocol database,
  handle management, and SMI dispatch used by the host implementation.

  Key structures:
    IHANDLE            — handle node in gSmmHandleList
    PROTOCOL_ENTRY     — protocol GUID entry in mSmmProtocolDatabase
    PROTOCOL_INTERFACE — links a protocol to a handle
    PROTOCOL_NOTIFY    — notification registration
    SMI_ENTRY/SMI_HANDLER — SMI dispatch (used by SmmServicesTableLibHost.c)

  Many function prototypes declared here are NOT implemented in the host
  library (e.g., SmmInitializeMemoryServices, SmramProfileInit, etc.).
  Only the functions actually called by the 5 source files are linked.
  The extraneous prototypes are harmless and kept for upstream compatibility.

  Includes a large set of edk2 headers; only a subset is actually needed
  for the host build. The unused includes don't cause issues because the
  host build system resolves them from edk2 packages.
...
#define SMI_HANDLER_SIGNATURE  SIGNATURE_32('s','m','i','h')
 ...
 typedef struct {
  ...
  BOOLEAN                       ToRemove;    // Deferred removal flag for SmiHandlerUnRegister during dispatch
} SMI_HANDLER;
...
extern LIST_ENTRY             gSmmHandleList;
