/** @file
  SmmInstallConfigurationTable — Host-based implementation.

  Copyright (c) 2009 - 2018, Intel Corporation. All rights reserved.<BR>
  SPDX-License-Identifier: BSD-2-Clause-Patent

  ## AUDIT ##
  Faithful copy of edk2/MdeModulePkg/Core/PiSmmCore/InstallConfigurationTable.c.
  Manages the SMST configuration table array (gSmmCoreSmst.SmmConfigurationTable).

  Public EFIAPI (wired into SMST):
    SmmInstallConfigurationTable — add, modify, or delete configuration table entries.
      - Table != NULL and GUID not found → add new entry
      - Table != NULL and GUID found    → modify existing entry
      - Table == NULL and GUID found     → delete entry
      - Table == NULL and GUID not found → EFI_NOT_FOUND

  Uses dynamic reallocation in blocks of CONFIG_TABLE_SIZE_INCREASED (16) entries.
  Correctly handles re-entrancy during FreePool by updating the table pointer
  before freeing the old table.

  Globals:
    mSmmSystemTableAllocateSize — current allocated size of the table array

  No modifications needed — logic matches upstream.
**/
