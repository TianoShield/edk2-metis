/**
  Trusted certificate database (certdb) management.

  AUDIT NOTES (AuthVarCertDB.c):
  ==============================
  - GetCertDB: Retrieves the "certdb" variable from the in-memory store.
  - FindCertsFromDb: Traverses certdb entries to find matching certs.
    [AUDIT FIX] Fixed CompareGuid logic — was checking == 0 (inverted).
  - DeleteCertsFromDb: Removes a cert entry from certdb.
  - InsertCertsToDb: Inserts a new cert entry.
    [AUDIT FIX] Zero-initialized Sha256Digest since the SHA-256
    computation is disabled (#if 0).
  These functions support the auth variable path which is largely disabled
  for fuzzing, but the fixes ensure correctness if the path is re-enabled.
  ...
  )
{
  ...
  while (Offset < CertDBListSize) {
    ...
    //
    // [AUDIT FIX] CompareGuid returns TRUE when equal, not 0.
    // The original code had (CompareGuid(...) == 0) which is backwards.
    //
  ...
  )
{
  EFI_STATUS                Status;
  //
  // [AUDIT FIX] Zero-initialize Sha256Digest since the actual SHA-256
  // computation is disabled (#if 0). Without this, garbage bytes
  // would be written into the certdb.
  //
  UINT8                     Sha256Digest[SHA256_DIGEST_SIZE] = {0};
  UINT8                     *NewBuffer;
  UINTN                     NewBufferSize;
  UINT32                    NameSize;
  UINT32                    NodeSize;
