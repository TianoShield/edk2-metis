/**
  NV variable store parsing utilities.

  AUDIT NOTES (Variable.c):
  =========================
  This file provides low-level accessors for navigating the binary layout of
  UEFI NV variable stores (as stored in flash images). Functions handle three
  variable header types: Normal, CountBasedAuth, TimeBasedAuth.

  Key functions:
    - GetStartPointer/GetEndPointer: Boundary accessors for the store.
    - IsValidVariableHeader: Checks the VARIABLE_DATA start magic.
    - NameSizeOfVariable/DataSizeOfVariable: Size accessors per type.
    - GetVariableNamePtr/GetVariableDataPtr: Pointer accessors.
    - GetNextVariablePtr: Iterates to the next variable entry.
    - GetVariableVar: Finds free space in an NV store.
    - FindVariableInNv: Searches by name+GUID in an NV store.

  Note: AttributesOfVariable always reads from .Normal.Attributes. This works
  because the Attributes field is at the same offset (byte 4) in all three
  union members. The code is correct but relies on the union layout.
