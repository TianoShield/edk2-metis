/** @file
  Variable.h - NV variable store header format definitions.

  AUDIT NOTES (Variable.h):
  =========================
  Defines the binary layout of UEFI variables in flash/NV storage:
    - VARIABLE_STORE_HEADER: Variable store region header with signature/size.
    - VARIABLE_HEADER_NORMAL: Standard variable header (no auth).
    - VARIABLE_HEADER_COUNT_BASED_AUTH: Count-based authenticated variable header.
    - VARIABLE_HEADER_TIME_BASED_AUTH: Time-based authenticated variable header.
    - VARIABLE_HEADER union: Unions all three header types via Common member.
  These structures are used by Variable.c for parsing NV variable images.
  For the host-based in-memory store (OsVariable.c), VARIABLE_INFO_PRIVATE
  is used instead — these NV structures are only needed if loading a variable
  image from a file.
